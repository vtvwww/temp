...
function fn_amazon_validate_request($processor_data, $request)
{
	if (!empty($processor_data['params']['aws_access_public_key'])) {
		$sign = fn_amazon_calculate_signature(urldecode($request['UUID']) . $request['Timestamp'], $processor_data['params']['aws_secret_access_key']);
...
function fn_prepare_to_place_order(&$xml_data, &$cart, &$auth)
{
	...
	$selected_shipping = preg_replace('/\(' . fn_get_lang_var('price_includes_tax') . '.*/i', '', $selected_shipping);	
