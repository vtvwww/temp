...
if ($message_recognizer == 'OrderCalculationsRequest') {
	...
	if (!empty($amazon_sess_id)) {
		Session::reset_id($amazon_sess_id);
	...
	if (!fn_amazon_validate_cart_data($cart, $xml)) {
		fn_set_notification('E', fn_get_lang_var('error'), fn_get_lang_var('text_amazon_incorrect_products_count'));
	...
	if ($tax_calculation_type == 'amazon') {
		foreach ($cart['taxes'] as $tax_id => $tax) {
			...
			$callback_response['TaxTables'] .= fn_array_to_xml(array(
				'TaxTable' => array(
	...
	$promotion_data = array(
		'Promotion' => array(
			...
			'Description' => fn_get_lang_var('discount'),
			'Benefit' => array(
	...
	if (empty($_SESSION['shipping_rates']) && $free_shipping && !isset($shipping_required)) {
		$_SESSION['shipping_rates'][] = array(
			'name' => fn_get_lang_var('free_shipping'),
			'delivery_time' => '',
			'rates' => array(
				'0' => 0,
			),
	...
	if ($calculate_shippings && !empty($_SESSION['shipping_rates'])) {
		// Prepare shipping methods callback
		$callback_response['ShippingMethods'] = '';
		
		foreach ($_SESSION['shipping_rates'] as $rate_id => $shipping) {
			...
			$shipping_data = array(
				'ShippingMethod' => array(
					...
					'DisplayableShippingLabel' => $shipping['name'] . (empty($tax_rate) ? '' : (' (' . fn_get_lang_var('price_includes_tax') . ': $' . fn_format_price($tax_rate) . ')')),
				),
			);
			
			$items_shipping .= fn_array_to_xml(array('ShippingMethodId' => $shipping['name'] . ' ' . $shipping['delivery_time']));
			$callback_response['ShippingMethods'] .= fn_array_to_xml($shipping_data);
		}
	}
	...
	foreach ($amazon_products as $key => $product) {
		...
		$callback_response['Response']['CallbackOrders']['CallbackOrder']['CallbackOrderItems'] .= fn_array_to_xml($item);
	}
	...
	if ($tax_calculation_type == 'default' && $tax_subtotal > 0) {
		...
		$callback_response['Response']['CallbackOrders']['CallbackOrder']['UpdatedCartItems'] .= fn_array_to_xml($tax);
		
		$tax = array(
			'UpdatedCartItem' => array(
				'SKU' => 'taxes',
				'MerchantId' => $processor_data['params']['merchant_id'],
				'Title' => fn_get_lang_var('taxes') . ': ' . substr($tax_description, 0, 240),
				'Price' => array(
					'Amount' => fn_format_price($tax_subtotal),
					'CurrencyCode' => $processor_data['params']['currency'],
				),
	...
	if ($processor_data['params']['aws_access_public_key']) {
		$sign = urlencode(fn_amazon_calculate_signature($callback_response, $processor_data['params']['aws_secret_access_key']));
		$aws_access_key = urlencode($processor_data['params']['aws_access_public_key']);
	...
	// Compare the cart data with the Amazon request
	if (!fn_amazon_validate_cart_data($cart, $xml)) {
		fn_set_notification('E', fn_get_lang_var('error'), 'text_amazon_incorrect_products_count');
