...
if (defined('PAYMENT_NOTIFICATION')) {
	...
	$callback_url = ($processor_data['params']['test'] == 'Y' ? Registry::get('config.http_location') : Registry::get('config.https_location')) . '/payments/amazon/amazon_callback.php';
	$cancel_url = Registry::get('config.http_location') . '/' . fn_url('checkout.cart');
	$return_url = Registry::get('config.http_location') . '/' . fn_url('payment_notification.placement?payment=amazon_checkout');
