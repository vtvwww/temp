...
if (defined('PAYMENT_NOTIFICATION')) {
	...
	include(DIR_PAYMENT_FILES . 'emerchantpay_files/ParamAuthenticate.inc');
	...
	if ($mode == 'process') {
		...
		$authenticatedParam = ParamAuthenticate($processor_data['params']['secret_key'], $_POST);
		...
		fn_update_order_customer_info($_REQUEST, $order_id);
		...
		$authenticatedParam = ParamAuthenticate($processor_data['params']['secret_key'], $_GET);
	...
	$payment_url =  $processor_data['params']['payment_form_url'] . '/payment/form/post';
	$payment_lang = fn_strtolower(CART_LANGUAGE);
	$payment_order = ($order_info['repaid']) ? ($order_id . '_'. $order_info['repaid']) : $order_id;
	$payment_currency = $processor_data['params']['currency'];
	$payment_mode = ($processor_data['params']['mode'] == 'TEST') ? 1 : 0;
	...
	include(DIR_PAYMENT_FILES . 'emerchantpay_files/ParamSigner.class');
	
	$ps = new Paramsigner;
	$ps->setSecret($processor_data['params']['secret_key']);

	$ps->setParam('client_id', $processor_data['params']['client_id']);
	$ps->setParam('form_id', $processor_data['params']['form_id']);
	...
	if (empty($order_info['use_gift_certificates']) && !floatval($order_info['subtotal_discount']) && empty($order_info['points_info']['in_use'])) {
		...
		if (!empty($order_info['gift_certificates'])) {
			foreach ($order_info['gift_certificates'] as $k => $v) {
				...
				$item_name = htmlspecialchars(fn_get_lang_var('gift_certificate'));
		...
		if (floatval($order_info['payment_surcharge'])) {
			$item_name = htmlspecialchars(fn_get_lang_var('surcharge'));
		...
		if (fn_order_shipping_cost($order_info)) {
			$item_name = htmlspecialchars(fn_get_lang_var('shipping_cost'));
...
function fn_update_order_customer_info($data, $order_id)
{
	...
	foreach ($customer_info_fields as $k => $v) {
		if (isset($data[$k]) && $data[$k] != $order_info[$v]) {
			$need_update = true;
			$new_order_info[$v] = $data[$k];
		}
	}
