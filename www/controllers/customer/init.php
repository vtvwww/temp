...
if (PRODUCT_TYPE == 'ULTIMATE') {
	if (!empty($_REQUEST['skey'])) {
		$session_id = fn_get_storage_data('session_' . $_REQUEST['skey']);
		fn_set_storage_data('session_' . $_REQUEST['skey'], '');
		...
		if (!empty($session_id)) {
			...
			Session::save($session_id, unserialize($session_data));
		}
		
		$redirect_url = fn_query_remove(REAL_URL, 'skey');
