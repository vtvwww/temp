...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if ($mode == 'update_payment' && !empty($_REQUEST['payment_id'])) {
		...
		if (!empty($_REQUEST['active_tab'])) {
			$_suffix .= '&active_tab=' . $_REQUEST['active_tab'];
		}
	}
	...
	if ($mode == 'update_steps') {
		...
		if (!empty($auth['user_id'])) {
			...
			if ($_REQUEST['update_step'] == 'step_two' || $_REQUEST['update_step'] == 'step_one' && !$errors) {
				...
				fn_update_user($auth['user_id'], $user_data, $auth, $_REQUEST['ship_to_another'], $send_notification, false);
...
// Step 2/3: Profile fields/Shippings methods
	...
	fn_add_breadcrumb(fn_get_lang_var('checkout'));
	...
	$view->assign('contact_fields_filled', $contact_fields_filled);

	// Check fields population on first and second steps
	if (($contact_info_population == true || ACTION == 'guest_checkout') && empty($_SESSION['failed_registration'])) {
	...
	} elseif (ACTION == 'guest_checkout' && !empty($_SESSION['failed_registration'])) {
		$completed_steps['step_one'] = true;
	}
	...
	if (!empty($_SESSION['shipping_rates'])) {
		...
		foreach ($rates as $shipping_id => $shipping) {
			...
			unset($rates[$shipping_id]['name']);
			unset($rates[$shipping_id]['delivery_time']);
	...
	if (!empty($payment_methods)) {
		...
		if (!empty($checkout_buttons)) {
			$view->assign('checkout_buttons', $checkout_buttons, false);
		}
	} else {
		$first_method = false;
	}
	...
	if (!empty($_REQUEST['active_tab'])) {
		...
		if (!empty($payment_methods[$active_tab]) && count($payment_methods[$active_tab]) == 1) {
			$cart['payment_id'] = key($payment_methods[$active_tab]);
		} elseif (!empty($payment_methods[$active_tab])) {
			if (!empty($cart['payment_id']) && !isset($payment_methods[$active_tab][$cart['payment_id']])) {
				$cart['payment_id'] = key($payment_methods[$active_tab]);
			}
		}
	}
	...
	if (empty($_SESSION['failed_registration']) && !defined('AJAX_REQUEST') && isset($_REQUEST['edit_step'])) {
		return array(CONTROLLER_STATUS_REDIRECT, fn_query_remove(Registry::get('config.current_url'), 'edit_step'));
	}
