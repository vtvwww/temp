...
if ($_SERVER['REQUEST_METHOD']	== 'POST') {
	...
	if ($mode == 'backup') {
		...
		$dbdump_filename = empty($_REQUEST['dbdump_filename']) ? 'dump_' . date('mdY') . '.sql' : basename($_REQUEST['dbdump_filename']);
		...
		foreach ($dbdump_tables as $k => $table) {
			if (!empty($_REQUEST['dbdump_schema']) && $_REQUEST['dbdump_schema'] == 'Y') {
				fn_set_progress('echo', '<br />' . fn_get_lang_var('backupping_schema') . ': <b>' . $table . '</b>');
			...
			if (!empty($_REQUEST['dbdump_data']) &&  $_REQUEST['dbdump_data'] == 'Y') {
				fn_set_progress('echo', '<br />' . fn_get_lang_var('backupping_data') . ': <b>' . $table . '</b>&nbsp;&nbsp;');
	...
	if ($mode == 'delete') {
		if (!empty($_REQUEST['backup_files'])) {
			foreach ($_REQUEST['backup_files'] as $file) {
				@unlink(DIR_DATABASE . 'backup/' . basename($file));
			}
		}
	}
