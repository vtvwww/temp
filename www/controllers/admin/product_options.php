...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if ($mode == 'update_combinations') {
		...
		$product = db_get_field("SELECT product FROM ?:product_descriptions WHERE product_id = ?i", $_REQUEST['product_id']);
		...
		if (!empty($_REQUEST['inventory'])) {
			foreach ($_REQUEST['inventory'] as $k => $v) {
				...
				if (($inventory[$k]['amount'] <= 0) && ($v['amount'] > 0)) {
					fn_send_product_notifications($_REQUEST['product_id'], $product);
				}
			}
		}
...
{
	$extra_variant_fields = $product_ids = $condition = $join = '';

	fn_set_hook('get_product_options', $extra_variant_fields, $product_ids, $lang_code, $condition, $join);
	...
	$opt = db_get_row(
		...
		. " WHERE a.option_id = ?i AND a.product_id = ?i"
		. " ORDER BY a.position",
		$lang_code, $option_id, $product_id
	);
	...
	if (!empty($opt)) {
		...
		if (PRODUCT_TYPE == 'ULTIMATE' && defined('COMPANY_ID')) {
			...
			$join .= db_quote(' LEFT JOIN ?:ult_product_option_variants shared_option_variants ON shared_option_variants.variant_id = a.variant_id AND shared_option_variants.company_id = ?i', COMPANY_ID);
		}
