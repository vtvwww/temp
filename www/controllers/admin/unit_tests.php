...
if ($mode == 'manage') {
	$view->assign('aviable_units', Tests_Manager::get_aviable_units());
}
...
if ($mode == 'test') {
	$units = Tests_Manager::get_aviable_units();
	$log = array();
	
	foreach ($units['core'] as $unit) {		
		$log = fn_array_merge($log, Tests_Manager::sort_by_status(Tests_Manager::run_unit(basename($unit)), basename($unit)));
	}
	
	foreach ($units['addons'] as $addon => $addon_units) {		
		foreach ($addon_units as $unit) {
			$log = fn_array_merge($log, Tests_Manager::sort_by_status(Tests_Manager::run_unit(basename($unit), $addon), basename($unit), $addon));
		}		
	}
...
foreach ($tests as $test){
	...
	$log[basename($test)] = $test_manager->run();
}
