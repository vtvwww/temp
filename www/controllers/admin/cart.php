	...
	if (!empty($carts_list) && is_array($carts_list)) {
		...
		if (PRODUCT_TYPE == 'ULTIMATE') {
			foreach ($carts_list as $key => $cart_data) {
				$all_cart_products[$key] = db_get_array(
					"SELECT SUM(amount) as count, SUM(amount) as sum, SUM(amount * price) as total"
					. " FROM ?:user_session_products"
				...
				if (!empty($all_cart_products[$key])) {
					$carts_list[$key]['cart_products'] = $all_cart_products[$key][0]['count'];
					$carts_list[$key]['cart_all_products'] = $all_cart_products[$key][0]['sum'];
					$carts_list[$key]['total'] = $all_cart_products[$key][0]['total'];
			...
			foreach ($carts_list as $key => $cart_data) {
				$all_cart_products[$key] = db_get_array(
					"SELECT SUM(amount) as count, SUM(amount) as sum, SUM(amount * price) as total"														                                        . " FROM ?:user_session_products"
...
function fn_get_carts($params, $items_per_page = 0)
{
	...
	if (PRODUCT_TYPE == 'ULTIMATE') {
		...
		$carts_list = db_get_array("SELECT SQL_CALC_FOUND_ROWS " . implode(', ', $fields) . " FROM ?:user_session_products $join WHERE 1 $condition $group ORDER BY $sorting $limit");
	} else {
		$carts_list = db_get_array("SELECT " . implode(', ', $fields) . " FROM ?:user_session_products $join WHERE 1 $condition $group ORDER BY $sorting $limit");
	}
