...
if ($mode == 'manage') {
	...
	if (defined('COMPANY_ID') && !empty($_REQUEST['user_type']) && ($_REQUEST['user_type'] == 'A' || $_REQUEST['user_type'] == 'P' && PRODUCT_TYPE == 'MULTIVENDOR')) {
		...
		if (PRODUCT_TYPE == 'MULTIVENDOR') {
			if (defined('COMPANY_ID')) {
				if (empty($_REQUEST['user_type'])) {
					return array(CONTROLLER_STATUS_REDIRECT, 'profiles.add?user_type=' . (COMPANY_ID > 0 ? 'V' : 'A'));
					...
					return array(CONTROLLER_STATUS_REDIRECT, 'profiles.add?user_type=V');
				}
			}
		}
	...
	if ($mode == 'update' && $_SERVER['REQUEST_METHOD'] != 'POST') {
		...
		if (PRODUCT_TYPE == 'MULTIVENDOR') {
			if (defined('COMPANY_ID')) {
				if (empty($_REQUEST['user_type'])) {
					return array(CONTROLLER_STATUS_REDIRECT, 'profiles.update?user_type=' . (COMPANY_ID > 0 ? 'V' : 'A'));
				} elseif ($_REQUEST['user_type'] == 'C') {
					return array(CONTROLLER_STATUS_DENIED);
				} elseif ($_REQUEST['user_type'] == 'V' && COMPANY_ID == 0) {
					return array(CONTROLLER_STATUS_REDIRECT, 'profiles.update?user_type=A');
				} elseif ($_REQUEST['user_type'] == 'A' && COMPANY_ID > 0) {
					return array(CONTROLLER_STATUS_REDIRECT, 'profiles.update?user_type=V');
				}
			}
		}
