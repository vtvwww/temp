...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if (($mode == 'update') || ($mode == 'add_tab')) {
		if (!empty($_REQUEST['tab_data'])) {
			...
			if (!isset($tab_data['tab_id']) || $tab_data['tab_id'] == 0) {
				$tab_data['position'] =  Bm_ProductTabs::instance()->get_max_position() + 1;;
			}

			Bm_ProductTabs::instance()->update($tab_data);
		}
	}
	...
	if ($mode == 'delete') {		
		if (!empty($_REQUEST['tab_id'])) {
			Bm_ProductTabs::instance()->delete($_REQUEST['tab_id']);
		}
	}
...
if ($mode == 'manage' || $mode == 'manage_in_tab') {
	...
	if (!empty($_REQUEST['dynamic_object'])) {
		...
		$dynamic_object_scheme = Bm_SchemesManager::get_dynamic_object_by_type($_REQUEST['dynamic_object']['object_type']);
		$selected_location = Bm_Location::instance()->get($dynamic_object_scheme['customer_dispatch'], $_REQUEST['dynamic_object'], DESCR_SL);
		$view->assign('location', $selected_location);
		$view->assign('dynamic_object_scheme', $dynamic_object_scheme);
	}

	$product_tabs = Bm_ProductTabs::instance()->get_list('', $product_id, DESCR_SL);

	$view->assign('product_tabs', $product_tabs);
} elseif ($mode == 'delete') {
	...
	$dynamic_object_scheme = Bm_SchemesManager::get_dynamic_object_by_type('products');
	$selected_location = Bm_Location::instance()->get('products.view', array(), DESCR_SL);
	$view->assign('location', $selected_location);
	$view->assign('dynamic_object_scheme', $dynamic_object_scheme);
	...
	if ($tab_id > 0 && empty($_REQUEST['tab_data']['content'])) {
		$tab_data = current(Bm_ProductTabs::instance()->get_list(db_quote(' AND ?:product_tabs.tab_id=?i', $tab_id), 0, DESCR_SL));
	} 
	...
	if (isset($tab_data['block_id']) && $tab_data['block_id'] > 0) {
		...
		$view->assign('block_data', Bm_Block::instance()->get_by_id($tab_data['block_id'], 0, $dynamic_object, DESCR_SL));
	}
	...
	if (!empty($_REQUEST['id']) && !empty($_REQUEST['status'])) {
		...
		$tab = Bm_ProductTabs::instance()->get_list(db_quote(" AND ?:product_tabs.tab_id=?i", $_REQUEST['id']), 0);
		$tab = current($tab);
		...
		if (!empty($_REQUEST['dynamic_object']['object_id']) && $_REQUEST['dynamic_object']['object_id'] > 0) {
			...
			Bm_ProductTabs::instance()->update(array(
				'tab_id' => $_REQUEST['id'],
