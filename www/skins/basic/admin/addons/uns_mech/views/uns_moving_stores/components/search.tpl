...
{capture name="section"}
    <form action="{""|fn_url}" name="search_form" method="get">
        ...
        <table cellpadding="10" cellspacing="0" border="0" class="search-header">
            <tr>
                <td colspan="2" class="nowrap search-field">
                    <label>Дата проведения документа:</label>
                    <div class="break">
                        {include file="common_templates/period_selector.tpl" period=$search.period prefix=""}
                    </div>
                </td>
                <td colspan="1" class="nowrap search-field b1_l">
                    <label for="o_id">Выбор Склада:</label>
                    <div class="break">
                ...
                <td colspan="1" class="nowrap search-field b1_l">
                    ...
                    <div class="break">
                        <select name="type" id="type">
                            <option value="0">---</option>
                            <option value="6" {if $search.type == 6}selected="selected"{/if}>Приходный ордер</option>
                            <option value="2" {if $search.type == 2}selected="selected"{/if}>Перемещение</option>
                            <option value="8" {if $search.type == 8}selected="selected"{/if}>Акт изменения остатка</option>
                        </select>
                    </div>
                </td>
            </tr>
