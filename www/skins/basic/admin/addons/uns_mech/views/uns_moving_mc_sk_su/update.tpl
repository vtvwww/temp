        ...
        <form action="{""|fn_url}" method="post" name="update_{$controller}_form_{$id}" class="cm-form-highlight">
            ...
            {capture name="tabsbox"}
                <div id="content_general">
                    ...
                    {include file="addons/uns_mech/views/uns_moving_mc_sk_su/components/document.tpl" d=$i}

                    {if $id>0}
                    {* Добавить позиции документа *}
                    {include file="addons/uns_mech/views/uns_moving_mc_sk_su/components/document_items.tpl" d=$i}
                    {/if}
                </div>
            {/capture}
