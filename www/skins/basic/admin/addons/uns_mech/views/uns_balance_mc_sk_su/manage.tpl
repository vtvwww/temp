...
{if $search.total_balance_of_details == "Y"}
    <table>
        <tr>
            <td valign="top"><b>Период:</b></td>
            <td>{$search.period|fn_get_period_name:$search.time_from:$search.time_to}</td>
        </tr>
        <tr>
            <td valign="top"><b>Объекты:</b></td>
            <td>{$objects_plain[10].path}<br>{$objects_plain[14].path}<br>{$objects_plain[17].path}</td>
        </tr>
    </table>
...
{include file="common_templates/mainbox.tpl" title="Баланс на `$last_date` [`$last_document_id`]" content=$smarty.capture.mainbox tools=$smarty.capture.tools}
