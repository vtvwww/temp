...
<div class="select-popup-container {$popup_additional_class}">
	...
	{/if}
		...
		{else}
			{if $status == "A"}
                {if $active_name}{$active_name}{else}{$lang.active}{/if}
			{elseif $status == "D"}
                {if $disabled_name}{$disabled_name}{else}{$lang.disabled}{/if}
			{elseif $status == "H"}
				{$lang.hidden}
			{elseif $status == "P"}
				{$lang.pending}
