...
<div class="form-field {if $editable}cm-no-hide-input{/if}">
{/if}

{if !$option.hide_label}
	<label for="{$html_id}"{if $option.required} class="cm-required"{/if}>{if $option.option_name}{$lang[$option.option_name]}{else}{$lang.$name}{/if}{if $option.tooltip}{include file="common_templates/tooltip.tpl" tooltip=$option.tooltip}{/if}:</label>
{/if}
{** Checkbox **}
{if $option.type == "checkbox"}
	<input type="hidden" name="{$html_name}" value="N" />
	<input type="checkbox" class="checkbox" name="{$html_name}" value="Y" id="{$html_id}" {if $value && $value == "Y" || !$value && $option.default_value == "Y"}checked="checked"{/if} />
{** Selectbox **}
{elseif $option.type == "selectbox"}
	...
	{foreach from=$option.values key="k" item="v"}
		<option value="{$k}" {if $value == $k}selected="selected"{/if}>{if $option.no_lang}{$v}{else}{$lang.$v}{/if}</option>
	{/foreach}
	</select>
...
{elseif $option.type == "input" || $option.type == "input_long"}
	<input id="{$html_id}" class="input-text{if $option.type == "input_long"}-long{/if}" name="{$html_name}" value="{if $value}{$value}{else}{$option.default_value}{/if}" />
...
{elseif $option.type == "text" || $option.type == "simple_text"}
	<textarea id="{$html_id}" name="{$html_name}" cols="55" rows="8" class="{if $option.type == "text"}cm-wysiwyg{/if} input-textarea-long">{$value}</textarea>
	{if $option.type == "text"}
		{* Process textarea with HTML editor *}
		<!--processForm-->
	{/if}
{elseif $option.type == "picker"} 
	{include file=$option.picker checkbox_name="block_items" 
...
{elseif $option.type == "enum"}
	{if $option.fillings}
		<div class="form-field {if $editable}cm-no-hide-input{/if}">
			<label for="block_{$html_id}_filling">{$lang.filling}:</label>
			<select id="block_{$html_id}_filling" name="block_data[content][{$name}][filling]" class="cm-reload-form">
				{foreach from=$option.fillings item=v key=k}
					<option value="{$k}" {if $block.content.$name.filling == $k}selected="selected"{/if}>{$lang.$k}</option>
				{/foreach}
			</select>
		...
		{if $filling == 'manually'}
			<div class="form-field {if $editable}cm-no-hide-input{/if}">
				{include file=$option.fillings.manually.picker checkbox_name="block_items" 
					data_id="objects_`$item.chain_id`_" 
