...
{elseif $user.company_id == $smarty.const.COMPANY_ID || "RESTRICTED_ADMIN"|defined || $auth.is_root == 'Y'}
...
<tr class="{cycle values="table-row, "}">
	...
	<td>
		...
		{if $user.is_root == "Y" && ($user.user_type == "A" || $user.user_type == "V" && "COMPANY_ID"|defined && $smarty.const.COMPANY_ID == $user.company_id)}
			{assign var="u_id" value=""}
		{else}
			{assign var="u_id" value=$user.user_id}
		{/if}
		{include file="common_templates/select_popup.tpl" id=$u_id status=$user.status hidden="" update_controller="profiles" notify=true notify_text=$lang.notify_user popup_additional_class=$popup_additional_class}
	</td>
	<td class="nowrap">
		{capture name="tools_items"}
		{hook name="profiles:list_extra_links"}
			{if $user.user_type == "C" && $user.is_editable == "Y"}
			...
			{if (!"COMPANY_ID"|defined && !($user.user_type == "A" && $user.is_root == "Y")) || ("COMPANY_ID"|defined && $user.user_type == "V" && $user.company_id == $smarty.const.COMPANY_ID && $user.is_root == "N")}
...
{capture name="tools"}
	...
	{else}
		...
		{foreach from=$user_types key="_k" item="_p"}
			{if !("COMPANY_ID"|defined && $smarty.const.PRODUCT_TYPE == 'MULTIVENDOR' && $_k == 'C')}
