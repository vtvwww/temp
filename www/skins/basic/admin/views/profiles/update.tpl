...
{capture name="tabsbox"}
	...
	<form name="profile_form" action="{""|fn_url}" method="post" class="cm-form-highlight{if "COMPANY_ID"|defined && $user_data.user_id && $user_data.company_id != $smarty.const.COMPANY_ID && $hide_inputs} cm-hide-inputs{/if}">
	{if $mode != "add"}
	...
	<div id="content_usergroups" class="cm-hide-save-button">
		...
		<tr {cycle values="class=\"table-row\", "}>
			...
			<td>
				...
				{assign var="lang_available" value=$lang.available|escape:dquotes}
				{assign var="lang_declined" value=$lang.declined|escape:dquotes}
				{include file="common_templates/select_popup.tpl" id=$usergroup.usergroup_id status=$ug_status hidden="" items_status="A: \"`$lang_active` \", P: \"`$lang_pending`\", F: \"`$lang_available`\", D: \"`$lang_declined`\"" extra="&amp;user_id=`$user_data.user_id`" update_controller="usergroups" notify=true hide_for_vendor="COMPANY_ID"|defined}
			</td>
		</tr>
		{foreachelse}
		<tr class="no-items">
	...
	<div class="buttons-container buttons-bg cm-toggle-button">
		{include file="buttons/save_cancel.tpl" but_name="dispatch[profiles.update]" hide_first_button=$hide_first_button hide_second_button=$hide_first_button}
	</div>
