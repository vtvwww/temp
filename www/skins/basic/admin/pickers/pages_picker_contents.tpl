...
<form action="{"`$index_script`?`$smarty.request.extra`"|fn_url}" rev="{$smarty.request.data_id}" method="post" name="pages_form">
	...
	{if $pages_tree}
		<div class="items-container multi-level">
			{include file="views/pages/components/pages_tree.tpl" header=true picker=true checkbox_name=$smarty.request.checkbox_name hide_delete_button=true display=$smarty.request.display dispatch="pages.picker" combination_suffix="_picker"}
		</div>
	{else}
