...
{capture name="add_buttons"}
	...
	{else}
		...
		{if !"COMPANY_ID"|defined || $smarty.const.CONTROLLER != "companies"}
			...
			{include file="buttons/button.tpl" but_id="opener_picker_`$data_id`" but_href="categories.picker?display=`$display`&amp;data_id=`$data_id`&amp;company_id=`$company_id`&amp;company_ids=`$company_ids``$extra_url`"|fn_url but_text=$_but_text but_role=$_but_role but_rev="content_`$data_id`" but_meta="cm-dialog-opener"}
