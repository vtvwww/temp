...
function fn_uninstall_addon($addon_name, $show_message = true)
{
	...
	if ($addon_scheme != false) {
		...
		fn_uninstall_addon_templates(basename($addon_name));

		if (file_exists(DIR_ADDONS . $addon_name . '/layouts.xml')) {
			$xml = simplexml_load_file(DIR_ADDONS . $addon_name . '/layouts.xml', 'SimpleXMLElement', LIBXML_NOCDATA);
...
function fn_install_addon($addon, $show_notification = true, $install_demo = false)
{
	...
	if ($addon_scheme != false) {
		...
		$settings['status'] = 'D';
		$settings['priority'] = $_data['priority'];
		Registry::set('addons.' . $addon, $settings);	
