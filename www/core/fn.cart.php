	...
	if (!empty($order_id)) {
		...
		if (!empty($order)) {
			...
			$order['items'] = db_get_hash_array(
				...
				. "AND ?:product_descriptions.lang_code = ?s WHERE ?:order_details.order_id = ?i ORDER BY ?:product_descriptions.product",
	...
	if (Registry::get('settings.General.tax_calculation') == 'unit_price') {
				...
				foreach ($taxes as $tax_id => $tax) {
					...
					if (isset($product['company_id'])) {
						if (!isset($subtotal[$tax_id]['companies'][$product['company_id']])) {
							$subtotal[$tax_id]['companies'][$product['company_id']] = 0;
						}
						$subtotal[$tax_id]['companies'][$product['company_id']] += $_subtotal;
						$priority_stack[$product['company_id']] = -1;
						$applied_taxes[$product['company_id']] = 0;
					}
				}
			}
		}
		...
		foreach ($subtotal as $tax_id => $_st) {
			...
			if (empty($_st['companies'])) {
				$_st['companies'][0] = $_st['applies']['P'];
			}
			
			foreach ($_st['companies'] as $company_id => $applies) {
				$apply_tax_stack = 0;
				
				if (!isset($priority_stack[$company_id])) {
					$priority_stack[$company_id] = -1;
				}
				
				if ($priority_stack[$company_id] < 0) {
					$priority_stack[$company_id] = $_st['priority'];
				} elseif ($priority_stack[$company_id] != $_st['priority']) {
					$apply_tax_stack = $applied_taxes[$company_id];
					$priority_stack[$company_id] = $_st['priority'];
				}
				...
				$products_tax = fn_calculate_tax_rates(array($_st), fn_format_price($applies * $ddc + $apply_tax_stack), 1, $auth, $cart);
				
				if (!isset($applied_taxes[$company_id])) {
					$applied_taxes[$company_id] = 0;
				}
				
				if ($_st['price_includes_tax'] != 'Y') {
					$applied_taxes[$company_id] += $products_tax[$tax_id]['tax_subtotal'];
				}
			...
			$calculated_data[$tax_id]['tax_subtotal'] = $calculated_data[$tax_id]['applies']['P'] + $shipping_tax[$tax_id]['tax_subtotal'];
...
{
	$tax['subtotal'] = $tax['applies']['P'] = $tax['applies']['S'] = $tax['applies']['PS'] = 0;
	$tax['applies']['items']['P'] = $tax['applies']['items']['S'] = $tax['applies']['items']['PS'] = array();
...
function fn_order_notification(&$order_info, $edp_data = array(), $force_notification = array())
{
	...
	if ($notify_user == true || $notify_department == true || $notify_supplier == true) {
		...
		if ($notify_user == true) {
			...
			fn_send_mail($order_info['email'], array('email' => $company['company_orders_department'], 'name' => $company['company_name']), 'orders/order_notification_subj.tpl', 'orders/order_notification.tpl', '', $order_info['lang_code']);
