...
function fn_get_skin_path($path = '[skin]/customer/', $zone = AREA_NAME, $company_id = null) {
	...
	fn_set_hook('get_skin_path_pre', $path, $zone, $company_id);
	$zone = $zone == 'mail' ? 'customer' : $zone;

	if (!isset($cache[$zone . $company_id])) {
		$cache[$zone . $company_id] = CSettings::instance()->get_value('skin_name_' . $zone, '', $company_id);
	}

	$skin_name = $cache[$zone . $company_id];
...
function fn_send_product_notifications($product_id, $product = '')
{
	...
	if (!empty($emails)) {
		...
		fn_send_mail($emails, Registry::get('settings.Company.company_orders_department'), 'product/back_in_stock_notification_subj.tpl', 'product/back_in_stock_notification.tpl', '', CART_LANGUAGE, Registry::get('settings.Company.company_orders_department')); 
...
function fn_get_setting_edition_condition($table, $strict = true)
{
	$edition_names = array(
		'COMMUNITY'    => 'PRO:',
		'PROFESSIONAL' => 'PRO:',
	...
	if ($strict && defined('COMPANY_ID') && PRODUCT_TYPE == 'ULTIMATE') {
	...
	foreach ($_edition_conditions as $ed_cond) {
		$edition_conditions[] = "FIND_IN_SET('$ed_cond', $table.edition_type)";
		$edition_conditions[] = "FIND_IN_SET('" . $edition_names[PRODUCT_TYPE] . $ed_cond . "', $table.edition_type)";
	}
	
	return '(' . implode(' OR ', $edition_conditions) . ')';
}
...
function fn_delete_static_data($param_id)
{
	...
	if (!empty($param_id)) {
		...
		if ($scheme[$section]['skip_edition_checking'] && defined('COMPANY_ID')) {
...
{
	$skin_path = fn_get_skin_path('[skins]/[skin]', $area);
...
/**
 * Function filters messages with links to helpdesk for rebrander's versions.
 * 
 * @param string $msg String with link
 * @return string String without link 
 */
function fn_filter_rebrand_data($msg)
{
	if (stripos(Registry::get('config.updates_server'), '') !== false) {
		$msg = preg_replace('/<a.*?\[helpdesk_url\].*?>/imS', '', $msg);
 ...
 * Removes languages
 * 
 * @param array List of 2 letters language code
 * @return bool Always true
 */
 ...
 * $default_lang language if all languages in cart disabled and
...
function fn_save_languages_integrity($default_lang = CART_LANGUAGE)
{
	...
	if (empty($is_customer_lang_avail)) {
		CSettings::instance()->update_value('customer_default_language', $first_avail_code, 'Appearance');
	}
	...
	if (empty($is_admin_lang_avail)) {
		CSettings::instance()->update_value('admin_default_language', $first_avail_code, 'Appearance');
	}
...
function fn_clone_language_values($table, $to_lang, $from_lang = CART_LANGUAGE)
{
	...
	db_query(
		"REPLACE INTO ?:$table (" . implode(', ', $fields_insert) . ") "
