...
function fn_companies_get_payouts($params = array())
{
	...
	$bcf_query = db_quote("SELECT SUM(payouts.order_amount) - SUM(payouts.payout_amount) * (-1) - SUM(payouts.commission_amount) AS BCF FROM ?:vendor_payouts AS payouts $join WHERE $condition AND payouts.start_date < ?i AND payouts.order_amount > ?i", $params['time_from'], 0);
