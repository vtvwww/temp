...
class Helpdesk {
	 ...
	 * @return bool
	 */
	public static function get_license_information($license_number = '')
	{
		...
		$request = array(
			'Request@action=check_license@api=3' => array(
				'token' => $token,
	...
	public static function token($generate = false)
	{
		if ($generate) {
			...
			CSettings::instance()->update_value('hd_request_code', $token);
		} else {
			$token = Registry::get('settings.hd_request_code');
		}
