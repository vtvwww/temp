...
class Bm_SchemesManager {
	...
	private static function _get_value($item)
	{
		...
		if (is_string($item) && self::function_exists($item)) {
			// If it's a function execute it return it return
			return call_user_func($item);
		} elseif (is_array($item) && !empty($item) && isset($item[0]) && is_string($item[0]) && function_exists($item[0])) { 
			// If it's a function execute it return it return
			$function_name = array_shift($function);
			return call_user_func_array($function_name, $function);
		} elseif (is_file($tpls)) {
			return array(
				strval($item) => array(
			...
			if (isset($item['data_function']) && self::function_exists($item['data_function'])) {
				$item['values'] = call_user_func($item['data_function']);
			}
			return $item;
		}
	}
	...
	public static function generate_template_name($path, $skin_path) 
	{
		...
		if (empty($name)) {
			$name = str_replace('.tpl', '', basename($path));
	 ...
	 * @static
	 * @param mixed $function Function name or array of class name and method name
	 * @return bool Returns true if function exists, false otherwise
	 */
	public static function function_exists($function)
	{
		return (is_string($function) && function_exists($function) || is_array($function) && isset($function[0]) && isset($function[1]) && method_exists($function[0], $function[1]));
	}
}
?>
