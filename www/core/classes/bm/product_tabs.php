...
class Bm_ProductTabs extends CompanySingleton {
	...
	{
		$path = fn_get_skin_path('[repo]/[skin]/customer/', 'customer', $this->_company_id);
		$path = $path . 'addons/' . $addon . '/blocks/product_tabs';

		$tabs_blocks = fn_get_dir_contents($path, false, true, '.tpl', 'addons/' . $addon . '/blocks/product_tabs/');	
		
		if (!empty($tabs_blocks)) {
			foreach($tabs_blocks as $template) {
				$name = fn_get_file_description($path . '/' . basename($template), 'block-description');
