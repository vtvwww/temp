...
class Bm_RenderManager {
	...
	public static function render_block_content($block)
	{
		...
		$cache_name = 'block_content' . $block['block_id'] . $block['snapping_id'] . $block['type'] . $block['grid_id'] . $block['object_id'] . $block['object_type'];
		...
		if (isset($block_scheme['cache']) && Registry::is_exist($cache_name) == true && self::allow_cache()) {
			...
			if (isset($block_scheme['cache']) && $display_block == true) {
	...
	private static function _render_main_content()
	{
		...
		$block_content = $smarty->fetch($smarty->get_var('content_tpl'), false);
	...
	public static function get_value($template_variable, $field, $block_scheme, $block)
	{
		...
		if ($field['type'] == 'function' && !empty($field['function']) && function_exists($field['function'][0])) {
			$value = call_user_func($field['function'][0], $value, $block, $block_scheme);
		}

		if (isset($field['handler']) && Bm_SchemesManager::function_exists($field['handler'])) {
			$value = call_user_func($field['handler'], $value, $block, $block_scheme);
		}
	 ...
	 * @return bool
	 */
	public static function delete_template_cache($delete)
	{
	...
	{
		return DIR_ROOT . '/' . Registry::get('customer_skin_path') . '/customer/';
	}
	...
	private static function _get_skin_path($area = 'C')
	{
		...
		if ($area == 'A') {
			$skin_path = DIR_SKINS . CSettings::instance()->get_value('skin_name_admin', '') . '/admin/views/block_manager/render/';
		} elseif ($area == 'C') {
			$skin_path = DIR_ROOT . '/' . Registry::get('customer_skin_path')  . '/customer/views/block_manager/render/';
		}
