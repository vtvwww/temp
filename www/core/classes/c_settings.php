...
class CSettings {
	...
	public function get_section_tabs($parent_section_id, $lang_code = CART_LANGUAGE) 
	{
		...
		foreach ($_tabs as $tab) {
			if (isset($this->sections[$parent_section_id]['name'])) {
				$func_name = 'fn_is_tab_' . fn_strtolower($this->sections[$parent_section_id]['name']) . '_' . $tab['name'] . '_aviable';
	...
	public function get_list($section_id = 0, $section_tab_id = 0, $company_id = null, $lang_code = CART_LANGUAGE)
	{
		...
		foreach($_settings as $setting) {
			...
			$setting['value'] = $this->unserialize($setting['value']);
		...
		if ($_result) {
			foreach ($_result as $_row) {
				...
				if (!empty($_row['section_tab_id']) && $hierarchy) {
					$settings[$section_name][$section_tab_name][$_row['name']] = $this->unserialize($_row['value']);
				} elseif (!empty($_row['section_id']) && $hierarchy) {
					$settings[$section_name][$_row['name']] = $this->unserialize($_row['value']);
				} else {
					$settings[$_row['name']] = $this->unserialize($_row['value']);
				}
			}
	...
	public function get_value($setting_name, $section_name, $company_id = null)
	{
		if (!empty($setting_name)) {
			...
			$_setting = $this->_get_list(
				array('?:settings_objects.object_id as object_id'),
				'', '', false, $condition, false, $company_id
			);

			if (isset($_setting[0]['value'])) {
				$value = $this->unserialize($_setting[0]['value']);
			} else {
				return false;
			}
	 ...
	 * @param bool $update_registry Update registry after setting was updated
	 * @return int Setting identifier if it was created, true un success update, false otherwise
	 */
	public function update($setting_data, $variants = null, $descriptions = null, $update_registry = false)
	{
	 ...
	 */
	public function update_value($setting_name, $setting_value, $section_name = '', $update_registry = false, $company_id = null)
	{
		if (!empty($setting_name)) {
	...
	public function update_value_by_id($object_id, $value, $company_id = null)
	{
		if (!empty($object_id)) {
			...
			if (defined('COMPANY_ID') || $company_id !== null) {
				if (strpos($edition_types, $edition_names[PRODUCT_TYPE] . CSettings::VENDOR) !== false) {
					$table = "settings_vendor_values";
					$data['company_id'] = (intval($company_id) > 0) ? $company_id : COMPANY_ID;
				}
			} else {
				if (strpos($edition_types, $edition_names[PRODUCT_TYPE] . CSettings::NONE) === false) {
	...
	/**
	 * Removes setting and all relaited data
	 * 
	 * @param string $section_name Setting name
	 * @param string $setting_name Section name
	 * @return bool Always true
	...
	/**
	 * Removes setting and all relaited data by id
	 * 
	...
	public function remove_by_id($setting_id)
	{
		...
		if (PRODUCT_TYPE == 'ULTIMATE' || PRODUCT_TYPE == 'MULTIVENDOR') {
			db_query('DELETE FROM ?:settings_vendor_values WHERE object_id = ?i', $setting_id);
		}
	...
	private function _get_list($fields, $section_id = '', $section_tab_id = '', $no_headers = false, $extra_condition = '', $is_global = true, $company_id = null, $lang_code = '')
	{
		...
		$condition .= ' AND ' . fn_get_setting_edition_condition('?:settings_objects', false);
		if ($no_headers) {
	...
	private function generate_edition_condition($table, $use_access_level = true) 
	{
		...
		foreach ($_edition_conditions as $edition_condition) {
			...
			$edition_conditions[] = "FIND_IN_SET('".$edition_names[PRODUCT_TYPE].$edition_condition."', $table.edition_type)";
		}
