...
class Addons_XmlScheme1 extends Addons_XmlScheme {
		...
		if (isset($this->_xml->opt_settings->section)) {
			foreach($this->_xml->opt_settings->section as $section) {
				if (isset($section['name']) && (string) $section['name'] != 'general') {
					...
					$name = fn_get_lang_var($id, $this->get_default_language());
				} else {
					$id = 'general';
					$name = 'General';
				}
				...
				$sections[] = array(
					'id' => $id,
					'name' => $name,
					'translations' => array(array('lang_code' => $this->get_default_language(), 'name' => $id, 'value' => $name)),
				...
				$sections[] = array(
					'id' => 'general',
					'name' => 'General',
					'translations' => array(array('lang_code' => $this->get_default_language(), 'name' => 'general', 'value' => 'General')),
					'edition_type' => 'ROOT'
				);			
		}
