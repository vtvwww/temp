...
class SmartyEngine_Core extends Smarty
{
	...
	{
		if ($this->debugging && basename($params['smarty_include_tpl_file']) == 'debug.tpl' || $params['smarty_include_tpl_file'] == 'evaluated template') {
			return true;
		}
    ...
    function _smarty_include($params)
    {
		...
		if (Registry::get('settings.store_optimization') == 'dev') {
			Bm_RenderManager::delete_template_cache($_is_compiled == false);
		}
	...
	function prefilter_keep_regexp_vars($content, &$compiler)
	{
		if (strpos($content, '$') !== false) {
			$content = preg_replace('/\$(\d{1,2})/S', '#####$1', $content);
		}
		return $content;
	}
	
	function outputfilter_keep_regexp_vars($content, &$compiler)
	{
		if (strpos($content, '#####') !== false) {
			$content = preg_replace('/#####(\d{1,2})/S', '\$$1', $content);
		}
