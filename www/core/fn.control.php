...
{
	Profiler::checkpoint('After init',0);
	...
	if ((defined('HTTPS') ? Registry::get('config.https_host') : Registry::get('config.http_host')) != REAL_HOST && $_SERVER['REQUEST_METHOD'] == 'GET' && !defined('CONSOLE')) {
		...
		fn_redirect($host . '/' . $path .(!empty($_SERVER['REDIRECT_URL']) ? $_SERVER['REDIRECT_URL'] : (!empty($_SERVER['REQUEST_URI']) ? $_SERVER['REQUEST_URI'] : Registry::get('config.current_url'))), false, true);
	}
	...
	if (!empty($_REQUEST['render_current']) && !empty($_SERVER['HTTP_REFERER'])) {
	...
	Profiler::checkpoint('Before TPL', 0);
	Registry::get('view')->display(Registry::get('root_template'));
	Profiler::checkpoint('Smarty TPL', 0);
	Profiler::display();
...
function fn_run_controller($path)
{
    ...
    Profiler::checkpoint("Before $controller", 0);
    $res_controller = include($path);
    Profiler::checkpoint(strtoupper($controller));
