...
function fn_get_mime_content_type($filename, $check_by_extension = true, $not_available_result = 'application/octet-stream')
{
	...
	if (empty($type) && $check_by_extension && strpos(basename($filename), '.') !== false) {
		$type = fn_get_file_type(basename($filename), $not_available_result);
	}
...
function fn_pathinfo($path, $encoding = 'UTF-8')
{
	...
	$dirname = fn_substr($path, 0, fn_strlen($path) - fn_strlen($basename) - 1, $encoding);
