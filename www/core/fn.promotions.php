...
function fn_promotion_apply($zone, &$data, &$auth = NULL, &$cart_products = NULL)
{
	...
	if ($zone == 'cart') {
		...
		if (!empty($applied_promotions)) {
			...
			if (!empty($_text)) {
				fn_set_notification('W', fn_get_lang_var('important'), fn_get_lang_var('text_applied_promotions') . ': ' . implode(', ', $_text), '', 'applied_promotions');
...
function fn_promotion_check_coupon(&$cart, $initial_check, $applied_promotions = array())
{
	...
	if ($initial_check == true) {
		...
		if (!empty($cart['coupons'][$cart['pending_coupon']])) {
			fn_set_notification('W', fn_get_lang_var('warning'), fn_get_lang_var('coupon_already_used'), '', 'promotion_coupon_already_used');
			...
			fn_set_notification('W', fn_get_lang_var('warning'), fn_get_lang_var('single_coupon_is_allowed'), '', 'promotion_single_coupon_is_allowed');
		...
		if (!empty($cart['pending_coupon'])) {
			...
			if (empty($coupon)) {
				if (!fn_notification_exists('W', 'error_coupon_already_used')) {
					fn_set_notification('W', fn_get_lang_var('warning'), fn_get_lang_var('no_such_coupon'), '', 'promotion_no_such_coupon');
