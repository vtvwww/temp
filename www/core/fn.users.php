...
function fn_is_restricted_admin($params)
{
	...
	if ($not_own_profile && isset($params['user_data']['user_type']) && fn_check_user_type_admin_area($params['user_data']['user_type'])) {
		return true;
	}
...
function fn_update_user($user_id, $user_data, &$auth, $ship_to_another, $notify_user, $send_password = false)
{
	...
	if (PRODUCT_TYPE == 'MULTIVENDOR') {
		...
		$is_root_admin_exists = db_get_field("SELECT user_id FROM ?:users WHERE company_id = ?i AND is_root = 'Y'", $user_data['company_id']);
	...
	fn_update_user_profile($user_id, $user_data, $action);
	...
	if (!empty($notify_user)) {
		...
		$index_script = fn_get_index_script($user_data);
		Registry::get('view_mail')->assign('login_url', Registry::get('config.http_location') . '/' . $index_script);
...
{
	$condition = fn_get_company_condition('?:users.company_id');
	$user_data = db_get_row("SELECT is_root, company_id, user_login FROM ?:users WHERE user_id = ?i $condition", $user_id);
