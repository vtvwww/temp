 ...
 * @retirn int Page identifier on success, false otherwise
 */
 ...
 * 
 * @param bool $only_avail if true get only aviable currensies
 * @return array Currencies list
 */
...
function fn_get_all_dispatches($lang_code = DESCR_SL)
{
	...
	if (Registry::is_exist('dispatch_list') == true) {
		...
		foreach($controllers as $controller) {
			...
			if (!empty($modes)) {
				$dispatch_list[basename($controller)] = fn_get_lang_var(basename($controller), $lang_code);
			}
			foreach ($modes as $mode) {
				$dispatch = basename($controller) . '.' . substr($mode, 0, -4);
 ...
 * @return bool True on success, false otherwise
 */
function fn_is_current_url($url, $active_for = '')
{
	$active = false;
 ...
 */
function fn_normalize_parce_url($url)
{
