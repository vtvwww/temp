$.extend({
	...
	initRangeSlider: function(elm)
	{
		...
		if (!elm.parents('.filter-wrap').children('.filter-title').hasClass('filter-open')) {
...
function form_handler(form)
{
	...
	this.check_fields = function()
	{
		...
		for (i=0; i < elms.length; i++) {
			...
			// Check for integer field
			if (lbl.hasClass('cm-integer-more-0')) {
                if (($.is.integer(elm.val()) == false) || (elm.val() <= 0 )) {
					if (lbl.hasClass('cm-required') || $.is.blank(elm.val()) == false) {
						this.form_message(lang.error_validator_integer + " И больше 0.", lbl);
						is_ok = false;
						set_mark = true;
					}
				}
...
function fn_parse_url(url)
{
	...
	for (var i = 0; i < pairs.length; ++i) {
		...
		result.params[decodeURIComponent(p[0])] = decodeURIComponent(p[1].replace(/\+/g, " "));
	}
