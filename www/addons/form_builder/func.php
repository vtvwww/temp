...
function fn_form_builder_update_page_post($page_data, $page_id, $lang_code = DESCR_SL)
{
	...
	if (!empty($form_values)) {
		...
		if (!empty($page_data['form']['elements'])) {
			...
			if ($result == true) {
				...
				$from = Registry::get('settings.Company.company_support_department');
				$is_html = true;
				...
				if ($result == true) {
					...
					fn_send_mail($page_data['form']['general'][FORM_RECIPIENT], $from, 'addons/form_builder/form_subject.tpl', 'addons/form_builder/form_body.tpl', $attachments, CART_LANGUAGE, $sender, $is_html);
				}
			}
		}
	}
