...
{if !$hide_form}
<form action="{""|fn_url}" method="post" name="product_form_{$obj_prefix}{$obj_id}" enctype="multipart/form-data" class="cm-disable-empty-files cm-required-form{if $settings.DHTML.ajax_add_to_cart == "Y" && !$no_ajax} cm-ajax cm-ajax-render-current{/if}">
	...
	{if $show_price_values && $show_old_price}
		...
		<span class="cm-reload-{$obj_prefix}{$obj_id}" id="old_price_update_{$obj_prefix}{$obj_id}">
			...
			{elseif $product.list_discount}
				<span class="list-price nowrap" id="line_list_price_{$obj_prefix}{$obj_id}">{if $details_page && !$quick_view}{$lang.list_price}: {/if}<strike>{include file="common_templates/price.tpl" value=$product.list_price span_id="list_price_`$obj_prefix``$obj_id`" class="list-price nowrap"}</strike></span>
			{/if}
