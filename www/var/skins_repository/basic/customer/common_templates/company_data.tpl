...
{capture name="company_descr_`$obj_id`"}
	{if $show_descr}
		{$company.company_description|unescape|strip_tags|truncate:1024}{if !$hide_links && $company.company_description|strlen > 1024} <a href="{"company.view?company_id=`$companies.company_id`"|fn_url}" class="lowercase">{$lang.more}</a>{/if}
	{/if}
{/capture}
{if $no_capture}
