{if $cart_agreements || $settings.General.agree_terms_conditions == "Y"}
	...
	{if $iframe_mode}
		...
		{
			if ($('form[name=payments_form_' + suffix + '] input:checkbox:checked').length > 0 && $('form[name=payments_form_' + suffix + '] input:checkbox:checked').length == $('form[name=payments_form_' + suffix + '] input:checkbox').length) {
	...
	//]]>
		...
		<div class="form-field margin-top terms">
			...
			<label for="id_accept_terms{$suffix}" class="valign cm-custom (check_agreement)"><input type="checkbox" id="id_accept_terms{$suffix}" name="accept_terms" value="Y" class="checkbox valign" {if $iframe_mode}onclick="fn_check_agreements('{$suffix}');"{/if} />{$lang.checkout_terms_n_conditions}</label>
			{/hook}
		</div>
		{/if}
		{if $cart_agreements}
