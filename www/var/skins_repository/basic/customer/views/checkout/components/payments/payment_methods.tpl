...
<div class="cm-tabs-content clearfix" id="tabs_content">
	{foreach from=$payment_methods key="tab_id" item="payments"}
		<div class="{if $active != $tab_id}hidden{/if}" id="content_payments_{$tab_id}">
			<form name="paymens_form_{$tab_id}" action="{""|fn_url}" method="post">
			<div class="checkout-billing-options">
				...
				{else}
					...
					{if $list_view}
						...
						{/foreach}
					{else}
						{include file="views/checkout/components/payments/payments_list.tpl" payment=$payments}
					{/if}
				{/if}
				...
				{if $iframe_mode}
					<div class="payment-method-iframe-box">
						...
						<div id="payment_method_iframe" class="payment-method-iframe">
