	...
	{/if}
		...
		{hook name="checkout:product_info"}
			...
			{elseif $product.discount|floatval || $product.taxes}
				<p><a rev="discount_{$key}" class="cm-dialog-opener cm-dialog-auto-size">{$lang.discount}</a></p>

				<div class="product-options hidden" id="discount_{$key}" title="{$lang.discount}">
