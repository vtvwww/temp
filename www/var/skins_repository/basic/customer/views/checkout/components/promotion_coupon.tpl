...
<div class="cm-tools-list code-input discount-coupon">
	{if $cart|fn_display_promotion_input_field}
		<form class="cm-ajax cm-ajax-render-current" name="coupon_code_form{$position}" action="{""|fn_url}" method="post">
		...
		{capture name="promotion_info"}
			{hook name="checkout:applied_coupons_items"}
				...
				<li>
					...
					{include file="buttons/button.tpl" but_href="checkout.delete_coupon?coupon_code=`$coupon_code`&redirect_url=`$_redirect_url`" but_role="delete" but_meta="cm-ajax cm-ajax-render-current" but_rev="checkout*"}
					</span>
				</li>
				{/foreach}
