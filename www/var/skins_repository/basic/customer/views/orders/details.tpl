...
{capture name="tabsbox"}
	...
	{if $view_only != "Y"}
		...
		<div class="orders-actions right">
			<ul>
			{if $view_only != "Y"}
...
<tr {$_class} valign="top">
	{if !$product.deleted_product && $product.product_status != "D"}
		{assign var="display_href" value=true}
	{/if}

	<td>{if $display_href}<a href="{"products.view?product_id=`$product.product_id`"|fn_url}" class="product-title">{/if}{$product.product|unescape}{if $display_href}</a>{/if}
	...
	<div id="content_shipment_info" class="orders-shipment {if $selected_section != "shipment_info"}hidden{/if}">
		{foreach from=$shipments key="id" item="shipment"}
			...
			{if $order_info.items.$product_hash}
				...
				{cycle values=",class=\"table-row\"" name="class_cycle" assign="_class"}
				<tr {$_class} valign="top">
					<td>{if !$product.deleted_product && !$product.shared_product && $product.product_status != "D"}<a href="{"products.view?product_id=`$product.product_id`"|fn_url}" class="product-title">{/if}{$product.product|unescape}{if !$product.deleted_product && !$product.shared_product && $product.product_status != "D"}</a>{/if}
						{if $product.extra.is_edp == "Y"}
						<div class="right"><a href="{"orders.order_downloads?order_id=`$order_info.order_id`"|fn_url}"><strong>[{$lang.download}]</strong></a></div>
						{/if}
						{if $product.product_code}
...
<div class="orders-communication">
