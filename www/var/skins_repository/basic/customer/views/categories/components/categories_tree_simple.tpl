	...
	<td width="97%">
		...
		<tr>
			<td class="nowrap" style="padding-left: {$_shift}px;">
				...
				{/if}</td>
			<td width="100%">
				<span id="category_{$cur_cat.category_id}" {if $cur_cat.has_children || $cur_cat.subcategories}class="strong"{/if}>{$cur_cat.category}</span>{if $cur_cat.status == "N"}&nbsp;<span class="small-note">-&nbsp;[{$lang.disabled}]</span>{/if}
			</td>
		</tr>
		</table>
	</td>
