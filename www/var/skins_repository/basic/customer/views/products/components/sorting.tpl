...
{math assign="min_range" equation="x*4" x=$settings.Appearance.columns_in_products_list|default:2}
{assign var="max_ranges" value=4}
{assign var="step" value=$min_range}
...
<span class="cm-dropdown-title sort-dropdown dropdown-wrap-left"><a class="dropdown-wrap-right">{$pagination.items_per_page} {$lang.per_page}</a></span>
	<ul class="cm-dropdown-content">
		{section loop=$max_ranges name="ranges"}
		...
		{/if}
		{math assign="step" equation="x*2" x=$step}
		{/section}
	</ul>
</div>
{/if}
