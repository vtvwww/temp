...
{if $view_mode != "list"}
	...
	{if !$no_container}<div class="buttons-container picker">{/if}{if $picker_view}[{/if}
		<div>
		{include file="buttons/button.tpl" but_id="opener_picker_`$data_id`" but_href="products.picker?display=`$display`&amp;picker_for=`$picker_for`&amp;extra=`$extra_var`&amp;checkbox_name=`$checkbox_name`&amp;aoc=`$aoc`&amp;data_id=`$data_id`" but_text=$but_text|default:$lang.add_products but_role=$but_role|default:"add" but_rev="content_`$data_id`" but_meta="cm-dialog-opener"}
