...
<div align="center">
	<ul id="scroll_list_{$block.block_id}" class="jcarousel-skin hidden">
		...
		{math equation="item_qty * cellspacing + image_h + text_h" assign="item_height" cellspacing=$cellspacing image_h=$image_h text_h=$text_h item_qty=$item_qty}
		...
		{foreach from=$items item="product" name="for_products"}
			...
			<tr>
				<td class="center compact"{if $block.properties.scroller_direction == "up" || $block.properties.scroller_direction == "down"} style="height: {$text_h}px;"{/if}>
