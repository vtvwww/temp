...
<div class="filter-wrap">
	...
	<span class="filter-title {if $hide_filter}filter-open{/if}"
		onclick="
			if ($('#content_product_more_filters_{$block.block_id}_{$filter.filter_id}').is(':hidden')) {ldelim}
				$.cookie.set('show_filter_{$block.block_id}_{$filter.filter_id}', 'true');
			{rdelim} else {ldelim}
				$.cookie.set('show_filter_{$block.block_id}_{$filter.filter_id}', 'false');
			{rdelim}
			$('#content_product_more_filters_{$block.block_id}_{$filter.filter_id}').slideToggle('fast');
			$(this).toggleClass('filter-open');
			$(this).parent().find('.toggle-arrow').toggleClass('open');
		">{$filter.filter}</span>
	{if $filter.slider}
	...
	{else}
		...
		{foreach from=$filter.selected_ranges name="selected_ranges" item="selected_range"}
			...
			<li>
				{strip}
					...
					<a href="{$href}" class="filter-item checked{if $use_ajax} cm-ajax cm-ajax-full-render" rev="{$ajax_div_ids}{/if}" rel="nofollow">{$filter.prefix}{$selected_range.range_name|fn_text_placeholders}{$filter.suffix}</a>
				{/strip}
			</li>
		{/foreach}
		...
		{if $smarty.foreach.ranges.iteration > $filter.display_count}
			<li class="extra-link-wrap">
				<a id="more_filters_{$block.block_id}_{$filter.filter_id}"
					onclick="
						if ($('#content_product_more_filters_{$block.block_id}_{$filter.filter_id} li.cm-toggle-visibility').is(':hidden')) {ldelim}
							$.cookie.set('more_filters_{$block.block_id}_{$filter.filter_id}', 'true');
							$(this).html('{$lang.less}');
						{rdelim} else {ldelim}
							$.cookie.set('more_filters_{$block.block_id}_{$filter.filter_id}', 'false');
							$(this).html('{$lang.more}');
						{rdelim}
						$('#content_product_more_filters_{$block.block_id}_{$filter.filter_id} li.cm-toggle-visibility').toggle('fast');
						$('#view_all_{$block.block_id}_{$filter.filter_id}').toggle('fast');
						return false;
					" class="extra-link">{if $smarty.cookies.$cookie_name != 'true'}{$lang.more}{else}{$lang.less}{/if}</a>
			</li>
		{/if}

		{if $filter.more_cut}
			{capture name="q"}{$filter_qstring|unescape}&filter_id={$filter.filter_id}&{if $smarty.request.features_hash}&features_hash={$smarty.request.features_hash}{/if}{/capture}
		...
		{if $filter.selected_ranges && $filter.ranges|fn_is_not_empty}
			...
			{*<p><a onclick="$('#other_variants_{$block.block_id}_{$filter.filter_id}').slideToggle('fast');" class="extra-link">{$lang.choose_other}</a></p>*}
		{/if}
...
<div class="filters-tools clearfix">
	...
	<a href="{if $smarty.request.category_id}{assign var="use_ajax" value=true}{*assign var="use_ajax" value=false*}{"categories.view?category_id=`$smarty.request.category_id`"|fn_url}{else}{assign var="use_ajax" value=false}{$index_script|fn_url}{/if}" rel="nofollow" class="reset-filters{if $use_ajax} cm-ajax cm-ajax-full-render" rev="{$ajax_div_ids}{/if}">{$lang.reset}</a>
	{/if}
</div>
