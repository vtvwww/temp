...
{/if}
$('input[name="gift_cert_data[send_via]"]').live('click', function() {$ldelim}
		$('#email_block').switchAvailability(); $('#post_block').switchAvailability();
	{$rdelim});
{$rdelim});
//]]>
</script>
...
<form {if $settings.DHTML.ajax_add_to_cart == "Y" && !$no_ajax && $mode != "update"}class="cm-ajax cm-ajax-render-current" {/if}action="{""|fn_url}" method="post" target="_self" name="gift_certificates_form">
{if $mode == "update"}
