...
{if $chains}
	...
	{foreach from=$chains key="key" item="chain"}
		...
		<form {if $settings.DHTML.ajax_add_to_cart == "Y" && !$no_ajax}class="cm-ajax cm-ajax-render-current"{/if} action="{""|fn_url}" method="post" name="chain_form_{$chain.chain_id}" enctype="multipart/form-data">
		...
		<div class="chain-content clearfix">
			...
			{if $chain.products}
				<div class="chain-product">
					...
					{include file="common_templates/popupbox.tpl" id="buy_together_options_`$chain.chain_id`_`$key`" link_meta="text-button" text=$lang.specify_options content=$smarty.capture.buy_together_product_options link_text=$lang.specify_options act="general"}
					</div>
					{/if}
					<div class="chain-note">
						{$chain.min_qty}&nbsp;x
						{if !(!$auth.user_id && $settings.General.allow_anonymous_shopping == "P")}
							{if $chain.price != $chain.discounted_price}
								<strike>{include file="common_templates/price.tpl" value=$chain.price}</strike>
							{/if}
							{include file="common_templates/price.tpl" value=$chain.discounted_price}
						{/if}
					</div>
				</div>
			{/if}
			...
			{foreach from=$chain.products key="_id" item="_product"}
				...
				<div class="chain-product">
					...
					{elseif $_product.aoc}
						...
						{/capture}
						<div class="chain-note">
						{include file="common_templates/popupbox.tpl" id="buy_together_options_`$chain.chain_id`_`$_product.product_id`" link_meta="text-button" text=$lang.specify_options content=$smarty.capture.buy_together_product_options link_text=$lang.specify_options act="general"}
						</div>
					{/if}
					<div class="chain-note">
						{$_product.amount}&nbsp;x
