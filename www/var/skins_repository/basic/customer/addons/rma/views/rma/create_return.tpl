...
{if $actions}
	<div class="rma-register-action float-left"><strong>{$lang.what_you_would_like_to_do}:</strong></div>
		<div>
			<select name="action">
...
<tr {cycle values=",class=\"table-row\""}>
	<td width="1%" class="center rma-register-id">
	...
	<td width="60%" class="left" ><a href="{"products.view?product_id=`$oi.product_id`"|fn_url}">{$oi.product|unescape}</a>
		{if $oi.product_options}
			{include file="common_templates/options_info.tpl" product_options=$oi.product_options}
		{/if}</td>
	<td width="1%" class="right nowrap">
...
<div class="rma-return-comments">
	<strong>Type comments here</strong>
