	...
	{strip}
		<span class="strong" style="padding-left: {$shift}px;">
			{if $category.has_children || $category.subcategories}
				...
				{else}
					<img src="{$images_dir}/plus.gif" width="14" height="9" border="0" alt="{$lang.expand_sublist_of_items}" title="{$lang.expand_sublist_of_items}" id="on_cat_{$category.category_id}" class="hand cm-combination" onclick="if (!$('#cat_{$category.category_id}').children().get(0)) $.ajaxRequest('{"categories.manage?category_id=`$category.category_id`"|fn_url:'A':'rel':'&'}', {$ldelim}result_ids: 'cat_{$category.category_id}'{$rdelim})" />
				{/if}
