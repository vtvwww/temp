...
{capture name="mainbox"}
    <form action="{""|fn_url}" method="post" name="{$controller}_form" class="{if ""|fn_check_form_permissions} cm-hide-inputs{/if}">
        ...
        <table cellpadding="0" cellspacing="0" border="0" width="100%" class="table">
            ...
            {foreach from=$mclasses item=i}
                <tr {cycle values="class=\"table-row\", "}>
                    {assign var="id" value=$i.mclass_id}
                    {assign var="value" value="mclass_id"}
                    {assign var="name" value=$i.mclass_name}
                    <td width="1%">
                        <input type="checkbox" name="mclass_ids[]" value="{$id}" class="checkbox cm-item" />
                    </td>
                    <td>{$i.mclass_name}</td>
                    <td>{$i.mclass_status}</td>
                    <td>{$i.mclass_position}</td>
                    ...
                    <td class="nowrap right">
                        ...
                        {include    file="common_templates/table_tools_list.tpl"
                                    id="`$controller``$id`"
                                    text="`$lang.uns_material_class`: `$name`</b>"
                                    act="edit"
    ...
    {capture name="tools"}
        {include file="common_templates/tools.tpl" tool_href="`$controller`.add" prefix="top" link_text=$lang.uns_new_material_class hide_tools=true}
    {/capture}
{/capture}
{include file="common_templates/mainbox.tpl" title=$lang.uns_material_classes content=$smarty.capture.mainbox tools=$smarty.capture.tools select_languages=true}
